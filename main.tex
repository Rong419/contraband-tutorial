...
\begin{figure}[!htbp]
    ...
    \caption{Choosing the relaxed uncorrelated clock model and initializing $c_m$ \textcolor{red}{[@Rong: update screenshot to use the standard UCLN model]}.}
    \label{fig:example5}
\end{figure}
...
Open the Tracer application and load file \textcolor{red}{\texttt{[PLACE EXACT FILE NAME HERE].log}}.
...
Finally, ``Sigmasq'' \textcolor{red}{[@Rong: If you have time, replace ``Sigmasq'' with the name you called this parameter in the other template]} should be initialized to 1.0 and not estimated.
...
Still in the ``Site Model'' panel, now select the discrete morphology partitions (\autoref{fig:mk}), and again choose ``Gamma Site Model'' with \textcolor{red}{[X categories @Rong: The figure shows 0 category counts for Lewis Mk -- what number of categories are we really using? Fill out here]}.
