...
\subsubsection{Loading the continuous morphological characters}\label{load-continuous-data}
...
\subsubsection{Loading tip dates (fossil ages)}\label{parse-fosill-age}
...
\subsubsection{Specifying prior models}\label{priors}
...
Then for ``Input Tree File'', choose \texttt{precooked-runs/carnivora\_continuous\_27\_tree\_bm.trees}, and for ``Output File'', type \texttt{carnivora\_continuous\_27\_bm\_mcc.tree}.
...
\subsubsection{Loading all the available data and setting the tip dates}

In this TED analysis we will do many things like we did for the first analysis with just continuous morphological characters.
The first thing we will do is to load the continuous morphology data as done before in section \ref{load-continuous-data}.

This time, however, we will also load the available molecular data by doing \emph{File \textgreater{}\textgreater{} Import Alignment}.
Then go to the ``Partitions'' tab, select the nucleotide entry, and in the bottom-left corner, click the ``Split'' button.
This will prompt you to specify how to partition the nucleotide sites in the molecular alignment (\autoref{fig:split}); here, choose ``{1,2} + 3'', which means the first and second codon positions should be treated as one partition and the third codon position as another (\autoref{fig:example8}).

The last type of data we will import is our discrete morphological characters, which we do by clicking \emph{File \textgreater{}\textgreater{} Add Morphological Data}.
...
\begin{figure}[!htbp]
    \centering
    \includegraphics[width=0.400000\textwidth]{figures/split.png}
    \caption{Menu for partitioning the molecular sequences.}
    \label{fig:split}
  \end{figure}
...
\begin{figure}[!htbp]
    ...
    \caption{All the data (after loading) to be use in the total-evidence dating analysis.}
    \label{fig:example8}
\end{figure}

Lastly, we specify our tip dates as described in section \ref{parse-fosill-age}.
...
\subsubsection{Shrinking the between-character correlation matrix, $\boldsymbol{\rho}$}

Because this analysis is more computationally onerous, we will estimate $\boldsymbol{\rho}$ from intraspecific data and then fix its value during MCMC.
To do so, we will employ technique from the regularization literature, described in section \ref{background}.

Click the ``Intraspecific'' tab (\autoref{fig:intraspecific}) and set the value of $\delta$ to 0.6364 (in a real analysis, $\delta$ would be estimated using methods implemented outside of BEAST2).
This parameter will not be sampled with MCMC, so we do not have to worry about assigning priors to it later on.

\begin{figure}[!htbp]
    \centering
    \includegraphics[width=0.700000\textwidth]{figures/ShrinkageModel.png}
    \caption{Loading the intraspecific continuous morphology data from 21 fox individuals.}
    \label{fig:intraspecific}
\end{figure}

The next step is to load the intraspecific continuous character data from 21 fox (\emph{Vulpes vulpes}) individuals.
Ideally, this data would be passed together with the same characters scored for all other carnivoran species, but see the disclaimer at the top of this document.
Instead, we will manually copy all the intraspecific character values, and paste them inside the ``Value'' box.
Inside the ``Dimension'' box, type 1827 -- this is the total number of character values we have ($21 \times 87$) -- and inside ``Minordimension'', type 87 (the number of different continuous characters in the data set).

Finally, ``Sigmasq''
% \textcolor{red}{[@Rong: If you have time, replace ``Sigmasq'' with the name you called this parameter in the other template]}
should be initialized to 1.0 and not estimated.
This setting effectively means we are not modeling the way different characters may or not be evolving faster or slower.

% In the "Shrinkage Model" panel, we will need to fill in three components of the model.
% First, the shrinkage parameter is given by a constant value in the box to the right of "Delta". Second, the continuous characters from 21 \textit{Vulpes vulpes} individuals are given in the block of "Population Traits". To be more specific, the trait data should be written in one-line data separated by spaces. In addition, the number of traits is given by "Minordimension" and should be consistent with the dimension of the continuous data in the ''Partitions" panel. Third, the added individual trait values are not only used for estimating correlations, but also normalizing the continuous data of the 19 carnivoran species. Therefore, we put a $\checkmark$ in the box in front of "Include Pop Var" (\autoref{fig:intraspecific}). 

\subsubsection{Configuring the different trait models}

Because this time we have two additional types of data -- a molecular and a discrete morphological alignment -- informing our parameters, we need to set up two additional models that will be accessible in the ``Site Model'' tab.
Click that tab, select the molecular data partitions, and choose ``Gamma Site Model'' from the top drop-down menu (set ``Gamma Category Count'' to 4), and ``HKY'' from the drop-down menu next to ``Subst model'' (\autoref{fig:hky}).

\begin{figure}[!htbp]
    \centering
    \includegraphics[width=0.700000\textwidth]{figures/SiteModelDNA.png}
    \caption{Setting a continuous-time Markov chain model for molecular data.}
    \label{fig:hky}
\end{figure}
  
Still in the ``Site Model'' panel, now select the discrete morphology partitions (\autoref{fig:mk}), and again choose ``Gamma Site Model'' with \textcolor{red}{[X categories]}.
For ``Subst model'', choose ``Lewis Mk''.

\begin{figure}[!htbp]
    \centering
    \includegraphics[width=0.700000\textwidth]{figures/SiteModelMk.png}
     \caption{Setting a continuous-time Markov chain model for discrete morphological data.}
    \label{fig:mk}
\end{figure}

(Note that this tab would be better named ``Trait Model'' and the ``Subst Model'' field would be more precisely named as ``Phylogenetic continuous-time Markov Chain model''.
Perhaps at some point BEAUti2 will be changed in that direction.)

\subsubsection{Setting up all the clock models}

As we did in section \ref{bm-clock-model}, and following the same logic from the previous section, we will assume relaxed uncorrelated clock models for each data partition.
The specifications are shown in \autoref{fig:example12}.
...
\subsubsection{Specifying priors models and the MCMC attributes}

We will specify priors and the MCMC procedure in the same way we did in sections \ref{priors} and \ref{specify-the-mcmc-chain-length-mcmc}.
After all is done, generate a new \texttt{.xml} for this analysis.
...
\subsection{Running the Analysis using BEAST2}\label{ted}

Now we can run the \texttt{.xml} file generated in the previous steps using BEAST2.
Because this analysis is even more time-consuming than the first one, even the more reason inspect a previously run MCMC analysis in the \texttt{precooked-runs/} folder.

\subsection{Analysing the results}

In this total-evidence dating analysis, we can inspect even more parameters in Tracer.

\clearpage

This time we have three global clock rates,

\begin{figure}[!htbp]
    \centering
    \includegraphics[width=0.700000\textwidth]{figures/results/EST_clock_rate.png}
    \caption{Posterior distribution of the global clock rates for all data partitions.}
    \label{fig:allglobal}
  \end{figure}

\noindent 

% the mean should be fixed at 1.0...
% \begin{figure}[!htbp]
%     \centering
%     \includegraphics[width=0.700000\textwidth]{figures/results/BM_clock_sigma.png}
...
\noindent and we can further look at the age of the root,

\begin{figure}[!htbp]
    \centering
    \includegraphics[width=0.700000\textwidth]{figures/results/EST_tree_height.png}
    \caption{Posterior distribution of the tree root age.}
     \label{fig:rootage}
\end{figure}
